    ...
    public class SecurityLoginRepository : IDataRepository<SecurityLoginPoco>
    {
        ...
        public IList<SecurityLoginPoco> GetAll(params Expression<Func<SecurityLoginPoco, object>>[] navigationProperties)
        {
            using (SqlConnection conn = new SqlConnection(_connStr))
            {
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = conn;
                cmd.CommandText = @"SELECT Id, Login, Password, Created_Date, Password_Update_Date, Agreement_Accepted_Date, Is_Locked, Is_Inactive, Email_Address, Phone_Number, Full_Name, Force_Change_Password, Prefferred_Language, Time_Stamp
                                    FROM   dbo.Security_Logins";
                conn.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                List<SecurityLoginPoco> SecurityLoginPocoList = new List<SecurityLoginPoco>();
                while (reader.Read())
                {
                ...
                conn.Close();
                return SecurityLoginPocoList.ToArray();
            }
        }
